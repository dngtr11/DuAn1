...
public class ViewThuongHieu extends javax.swing.JFrame {
    ...
    public ViewThuongHieu() {
        ...
        load();
        if(tblTH.getRowCount()>0){
            tblTH.setRowSelectionInterval(0, 0);
            showF();
        }
    }
    ...
    private void btnSuaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSuaActionPerformed
        ...
        int selectedRow = tblTH.getSelectedRow();
        if (selectedRow == -1) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn một hàng từ bảng để sửa.");
            return;
        }
        
        int id = (int) tblTH.getValueAt(selectedRow, 0); // Lấy ID từ bảng
        String maTH = txtMa.getText(); // Lấy mã màu từ JTextField
        String tenTH = txtTen.getText(); // Lấy tên màu từ JTextField
        ...
        String result = thr.updateEntity(th, id);
...
//            Logger.getLogger(Main.class.getName()).log(Level.SEVERE, null, ex);
   ...
   private void load() {
    mol.setRowCount(0);
    ArrayList<ThuongHieu>lst = thr.getALL(); // Sử dụng biến lst toàn cục
    ...
    if (lst != null) {
        for (ThuongHieu th : lst) {
            mol.addRow(new Object[]{
                ...
                th.getMaTH(),
                th.getTenTH(),
                th.isTrangThai() ? "Đang hoạt động" : "Đã ngừng"
            });
        }
    } else {
  ...
  private void showF(){
    ...
    txtMa.setText(tblTH.getValueAt(index, 1).toString());
    txtTen.setText(tblTH.getValueAt(index, 2).toString());
    cboTT.setSelectedItem(tblTH.getValueAt(index, 3));
   ...
   private boolean checkT(String ma){
    for(int i= 0;i<tblTH.getRowCount()-1;i++){
        if(tblTH.getValueAt(i, 1).toString().equals(ma)){
            return true;
        }
    }
    return false;
}
    private ThuongHieu getF(){
      ThuongHieu th = new ThuongHieu();
     
    th.setMaTH(txtMa.getText());
    th.setTenTH(txtTen.getText());
